...
class InteractionWithExpectedOutput(TypedDict):
    """schema for interaction with expected output, which will be used for test cases that require ground truth."""
    session_id: str
    input: str
    actual_output: str
...
questions_with_truth = [q1,q2,q3,q4,q5]

def prepare_interaction(interaction:Interaction)->InteractionWithExpectedOutput:
    ...
    for question in questions_with_truth:
        if question["question"] == interaction["user_query"]:
...
save_interactions("ground truth questions")
